...
async def update_inventory(product_id: str, data: dict, current_user: dict = Depends(get_current_user)):
    ...
    # Allow None for unlimited stock
    ...
    # If inventory_count is None, it means unlimited stock
    update_data = {
        "inventory_count": inventory_count  # None = unlimited, number = limited stock
    }
    
    # Only set out_of_stock if inventory reaches 0 (not for None/unlimited)
    if inventory_count is not None and inventory_count == 0:
        update_data["out_of_stock"] = True
    elif inventory_count is not None and inventory_count > 0:
        update_data["out_of_stock"] = False
...
async def share_product_with_meta(product_id: str, request: Request):
    ...
    try:
        ...
        # Build absolute URLs
        base_url = os.getenv('REACT_APP_BACKEND_URL', 'https://email-subscribe-test.preview.emergentagent.com')
